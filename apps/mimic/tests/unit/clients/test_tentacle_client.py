"""Unit tests for Tentacle client"""
...
@pytest.fixture
def tentacle_client():
    """Create Tentacle client instance"""
    with patch('src.clients.tentacle_client.settings') as mock_settings:
        mock_settings.TENTACLE_URL = "http://localhost:8000"
        mock_settings.TENTACLE_API_KEY = "test-api-key"
        mock_settings.TENTACLE_TIMEOUT_SECONDS = 30
        yield TentacleClient()


@pytest.mark.unit
@pytest.mark.asyncio
async def test_send_notification_success(tentacle_client, db_session, test_user_annual):
    ...
    with patch('src.clients.tentacle_client.SessionLocal', return_value=db_session), \
        ...
        result = await tentacle_client.send_notification(
            user_id=test_user_annual.id,
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_send_notification_no_provider_key(tentacle_client, db_session, test_user_annual):
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_get_workflow_status(tentacle_client):
    ...
    with patch('httpx.AsyncClient') as mock_client:
        ...
        result = await tentacle_client.get_workflow_status("workflow-123")
...
@pytest.mark.unit
def test_get_provider_credentials(tentacle_client, test_provider_key):
    ...
    credentials = tentacle_client._get_provider_credentials(test_provider_key)
