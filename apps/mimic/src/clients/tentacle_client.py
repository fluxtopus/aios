...
class TentacleClient:
    """HTTP client for interacting with Tentacle workflow API"""
    
    def __init__(self):
        self.base_url = settings.TENTACLE_URL
        self.api_key = settings.TENTACLE_API_KEY
        self.timeout = 30.0
    ...
    async def send_notification(
        ...
        """Send a notification via Tentacle workflow"""
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=self.timeout, follow_redirects=True) as client:
                ...
                if not run_id:
                    ...
                    raise ValueError("Failed to get workflow run ID from Tentacle response")
    ...
    async def trigger_workflow(
        ...
        """Trigger a compiled workflow in Tentacle"""
        ...
        try:
            ...
            # First, register the workflow spec in Tentacle
            ...
            # Call Tentacle to create workflow run from spec
            # Note: This assumes the workflow spec is already registered in Tentacle
    ...
    async def get_workflow_status(self, workflow_id: str) -> Optional[Dict[str, Any]]:
        """Get workflow execution status from Tentacle"""
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
