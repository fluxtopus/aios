...
Tentacle is the task orchestration backend for **Tasks**, **Inbox (Flux)**, **Plugins**, **DB-based Agents**, and **Integrations/Triggers**.

> Workflow YAML specs are deprecated. Durable execution is task-driven.

## Fast Start (Standalone, 5 minutes)

This path runs Tentacle without InkPass or Mimic.
...
- Exposes FastAPI endpoints consumed by `tentacle-ui`

## Architecture (DDD)

Tentacle follows layered DDD boundaries:
...
docker compose run --rm tentacle python -m pytest -q --ignore=tests/e2e

# Architecture guardrails
docker compose run --rm tentacle python -m pytest -q tests/architecture

# OpenAPI smoke
docker compose run --rm tentacle python -c "from fastapi.testclient import TestClient; from src.api.app import app; r=TestClient(app).get('/openapi.json'); print(r.status_code)"
