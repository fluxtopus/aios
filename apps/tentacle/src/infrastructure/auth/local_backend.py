# REVIEW: Relies on TENTACLE_API_KEY and TENTACLE_SECRET_KEY env vars;
# REVIEW: no rotation or revocation support.
"""Local authentication backend for standalone mode.

Provides JWT validation using TENTACLE_SECRET_KEY and API key
validation using TENTACLE_API_KEY. No external dependencies.
"""
...
class LocalAuthBackend(AuthBackend):
    ...
    - Validates JWTs using TENTACLE_SECRET_KEY (reuses src/api/auth.decode_token)
    - Validates API keys by comparing against TENTACLE_API_KEY env var
    ...
    def __init__(self) -> None:
        self._api_key = os.getenv("TENTACLE_API_KEY", "")
        logger.info(
    ...
    async def validate_token(self, token: str) -> Optional[Any]:
        """Decode JWT locally using TENTACLE_SECRET_KEY."""
        try:
    ...
    async def validate_api_key(self, api_key: str) -> Optional[Any]:
        """Compare against TENTACLE_API_KEY env var."""
        if not self._api_key:
            logger.debug("No TENTACLE_API_KEY configured, rejecting API key")
            return None
    ...
    async def create_api_key(
        ...
        raise NotImplementedError(
            "API key creation is not supported in standalone mode. "
            "Set TENTACLE_API_KEY environment variable instead."
        )
