    ...
    async with aiohttp.ClientSession() as session:
        async with session.get('http://api:8000/metrics') as response:
            if response.status == 200:
                ...
                resource_metrics = {
                    "tentacle_memory_usage_bytes": "Memory usage",
                    "tentacle_cpu_usage_percent": "CPU usage",
                    "tentacle_disk_usage_bytes": "Disk usage",
                    "tentacle_disk_io_operations_total": "Disk I/O",
                    "tentacle_network_bytes_total": "Network traffic",
                    "tentacle_open_file_descriptors": "File descriptors",
                    "tentacle_thread_count": "Thread count"
                }
                ...
                system_lines = [l for l in content.split('\n') 
                               if 'tentacle_system_info' in l and not l.startswith('#')]
                if system_lines:
