...
This example shows how Tentacle monitors CPU, memory, disk, and network resources.
"""
        ...
        async with session.get('http://api:8000/metrics') as response:
            if response.status == 200:
                ...
                memory_lines = [l for l in content.split('\n') 
                               if 'tentacle_memory_usage_bytes{' in l]
                ...
                cpu_lines = [l for l in content.split('\n') 
                            if 'tentacle_cpu_usage_percent{' in l]
                ...
                disk_lines = [l for l in content.split('\n') 
                             if 'tentacle_disk_usage_bytes{path="root"' in l]
                disk_values = {}
                ...
                fd_lines = [l for l in content.split('\n') 
                           if 'tentacle_open_file_descriptors{' in l]
...
async def main():
    ...
    print("This demo shows real-time resource monitoring in Tentacle")
