...
Usage:
    docker compose exec tentacle python -m src.examples.workspace_events_demo
...
Required environment variables:
    ...
    TENTACLE_URL: (optional) Tentacle service URL.
"""
...
TENTACLE_URL = os.environ.get("TENTACLE_URL", "http://tentacle:8000")
PLUS_USER = {
...
async def create_event(client: httpx.AsyncClient, token: str, event_data: dict) -> dict:
    ...
    response = await client.post(
        f"{TENTACLE_URL}/api/workspace/objects",
...
async def list_events(client: httpx.AsyncClient, token: str) -> list:
    ...
    response = await client.get(
        f"{TENTACLE_URL}/api/workspace/events",
        headers={"Authorization": f"Bearer {token}"}
    )
    response.raise_for_status()
    return response.json()
...
async def query_events(client: httpx.AsyncClient, token: str, query: dict) -> list:
    ...
    response = await client.post(
        f"{TENTACLE_URL}/api/workspace/objects/query",
        headers={"Authorization": f"Bearer {token}"},
...
async def search_events(client: httpx.AsyncClient, token: str, search_term: str) -> list:
    ...
    response = await client.post(
        f"{TENTACLE_URL}/api/workspace/objects/search",
        headers={"Authorization": f"Bearer {token}"},
