...
async def test_metrics_debug():
    ...
    agent_lines = [l for l in metrics.split('\n') if 'tentacle_agent_executions_total' in l and 'test-123' in l]
    ...
    # Check metrics again
    metrics = generate_latest().decode()
    agent_lines = [l for l in metrics.split('\n') if 'tentacle_agent_executions_total' in l and 'debug-456' in l]
    print(f"Found {len(agent_lines)} metric lines")
    for line in agent_lines:
        print(f"  {line}")
    ...
    # Check metrics
    metrics = generate_latest().decode()
    agent_lines = [l for l in metrics.split('\n') if 'tentacle_agent_executions_total' in l and agent.id in l]
