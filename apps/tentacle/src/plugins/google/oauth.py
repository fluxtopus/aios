...
async def google_oauth_start_handler(inputs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        state_secret = os.getenv("GOOGLE_OAUTH_STATE_SECRET") or os.getenv("TENTACLE_SECRET_KEY")
        if not state_secret:
            raise GoogleOAuthError("GOOGLE_OAUTH_STATE_SECRET (or TENTACLE_SECRET_KEY) must be configured")
        signature = hmac.new(
            state_secret.encode(),
            payload_b64.encode(),
            hashlib.sha256,
