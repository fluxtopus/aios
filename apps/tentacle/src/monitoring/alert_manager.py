"""
Alert management and notification routing for Tentacle.
...
class AlertManager:
    ...
    async def _send_email_notification(self, config: NotificationConfig, 
        ...
        # Send email
        msg = MIMEMultipart()
        msg['From'] = config.config.get("from_address", "tentacle@alerts.local")
    ...
    async def _send_pagerduty_notification(self, config: NotificationConfig, 
        ...
        if other_events:
            summary = f"Tentacle: {len(other_events)} alerts"
            custom_details = {
    ...
    async def _send_single_pagerduty_event(self, config: NotificationConfig,
        ...
        payload = {
            "routing_key": config.config["integration_key"],
            "event_action": action,
            "dedup_key": f"tentacle:{event.alert.name}",
            "payload": {
    ...
    def _build_email_subject(self, events: List[NotificationEvent]) -> str:
        ...
        if len(events) == 1:
            event = events[0]
            return f"[Tentacle Alert] {event.alert.severity.upper()}: {event.alert.name}"
        else:
            ...
            return f"[Tentacle Alert] {len(events)} alerts - Highest: {highest.upper()}"
    ...
    def _build_email_body(self, events: List[NotificationEvent]) -> str:
        ...
        <body style="font-family: Arial, sans-serif;">
            <h2>Tentacle Alert Notification</h2>
        ...
        html += """
            ...
            <p style="margin-top: 20px; font-size: 12px; color: #666;">
                This is an automated message from Tentacle monitoring system.
            </p>
        </body>
        </html>
        """
        ...
        blocks = [
            {
                ...
                "text": {
                    "type": "plain_text",
                    "text": "🚨 Tentacle Alert Notification"
                }
            }
        ]
