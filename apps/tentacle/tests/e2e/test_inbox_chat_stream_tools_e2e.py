...
Requires:
- inkpass and tentacle services running
- OPENROUTER_API_KEY configured in tentacle runtime
"""
...
def _get_default_tentacle_url() -> str:
    try:
...
class TestInboxChatStreamToolsE2E:
    def test_chat_stream_emits_tool_execution_event(self, auth_token: Optional[str]) -> None:
        ...
        with httpx.Client(timeout=timeout, headers=headers) as client:
            with client.stream(
                "POST",
                f"{TENTACLE_URL}/api/inbox/chat/stream",
                json=body,
            ) as response:
