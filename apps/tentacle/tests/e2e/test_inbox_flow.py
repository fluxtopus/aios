...
`docker compose exec tentacle python -m pytest tests/e2e/test_inbox_flow.py`.
...
TENTACLE_URL = os.environ.get("TENTACLE_URL", "http://localhost:8005")
...
def task_and_inbox(api):
    ...
    inbox_resp = api.get(f"{TENTACLE_URL}/api/inbox")
...
class TestInboxFlowE2E:
    ...
    def test_04_unread_count_at_least_one(self, api, task_and_inbox):
        ...
        resp = api.get(f"{TENTACLE_URL}/api/inbox/unread-count")
    ...
    def test_05_thread_has_messages(self, api, task_and_inbox):
        ...
        resp = api.get(f"{TENTACLE_URL}/api/inbox/{conv_id}/thread")
        assert resp.status_code == 200
        thread = resp.json()
    ...
    def test_08_mark_as_read(self, api, task_and_inbox):
        ...
        conv_id = task_and_inbox["conversation_id"]
        resp = api.patch(
            f"{TENTACLE_URL}/api/inbox/{conv_id}",
    ...
    def test_10_follow_up_creates_new_task(self, api, task_and_inbox):
        ...
        conv_id = task_and_inbox["conversation_id"]
        resp = api.post(
            f"{TENTACLE_URL}/api/inbox/{conv_id}/follow-up",
    ...
    def test_12_archived_excluded_from_default_list(self, api, task_and_inbox):
        ...
        resp = api.get(f"{TENTACLE_URL}/api/inbox")
        assert resp.status_code == 200
        data = resp.json()
