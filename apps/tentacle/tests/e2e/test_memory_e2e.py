...
Requires: inkpass, tentacle, postgres, redis all running.

NOTE: This test runs INSIDE the tentacle container via
`docker compose exec tentacle python -m pytest tests/e2e/test_memory_e2e.py`.
...
def _get_default_tentacle_url():
    """Get Tentacle URL - internal Docker network if running in container."""
    import socket
    try:
        socket.gethostbyname("tentacle")
        return "http://tentacle:8000"
    except socket.gaierror:
        return "http://localhost:8005"


INKPASS_URL = os.environ.get("INKPASS_URL", _get_default_inkpass_url())
TENTACLE_URL = os.environ.get("TENTACLE_URL", _get_default_tentacle_url())
...
def task_with_memory_context(api, stored_memories):
    ...
    create_resp = api.post(
        f"{TENTACLE_URL}/api/tasks/with-steps",
        json={
            "goal": goal,
            "steps": [
                {
    ...
    start_resp = api.post(f"{TENTACLE_URL}/api/tasks/{task_id}/start")
...
class TestMemoryE2E:
    ...
    def test_02_memories_retrievable_by_topic(self, api, stored_memories):
        ...
        resp = api.get(f"{TENTACLE_URL}/api/memories?topic=content")
    ...
    def test_04_task_exists_and_processing(self, api, task_with_memory_context):
        ...
        resp = api.get(f"{TENTACLE_URL}/api/tasks/{task_id}")
    ...
    def test_08_cleanup_memories(self, api, stored_memories):
        ...
        if brand_voice.get("id"):
            delete_resp = api.delete(
                f"{TENTACLE_URL}/api/memories/{brand_voice['id']}"
            )
        ...
        if target_audience.get("id"):
            delete_resp = api.delete(
                f"{TENTACLE_URL}/api/memories/{target_audience['id']}"
            )
            assert delete_resp.status_code in (204, 404), (
