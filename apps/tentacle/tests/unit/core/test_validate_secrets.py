...
class TestValidateSecrets:
    ...
    def _call_validate_secrets(self, app_env="development", secret_key=None, tentacle_secret_env=None):
        ...
        env = {}
        if tentacle_secret_env is not None:
            env["TENTACLE_SECRET_KEY"] = tentacle_secret_env

        with patch("src.core.config.settings", mock_settings), \
             patch.dict(os.environ, env, clear=False):
            # Remove TENTACLE_SECRET_KEY from env if not provided
            if tentacle_secret_env is None and "TENTACLE_SECRET_KEY" in os.environ:
                del os.environ["TENTACLE_SECRET_KEY"]
    ...
    def test_production_raises_when_tentacle_secret_env_is_insecure(self):
        """In production, insecure TENTACLE_SECRET_KEY env var should raise."""
        with pytest.raises(RuntimeError, match="TENTACLE_SECRET_KEY"):
            self._call_validate_secrets(
                app_env="production",
                secret_key="a-very-secure-random-key-that-is-long-enough-1234567890",
                tentacle_secret_env="change-this-in-production"
            )

    def test_production_raises_when_tentacle_secret_env_too_short(self):
        """In production, short TENTACLE_SECRET_KEY should raise."""
        with pytest.raises(RuntimeError, match="TENTACLE_SECRET_KEY"):
            self._call_validate_secrets(
                app_env="production",
                secret_key="a-very-secure-random-key-that-is-long-enough-1234567890",
                tentacle_secret_env="short"
            )
    ...
    def test_production_passes_when_both_keys_secure(self):
        """Both SECRET_KEY and TENTACLE_SECRET_KEY secure should pass."""
        self._call_validate_secrets(
            app_env="production",
            secret_key="a-very-secure-random-key-that-is-long-enough-1234567890",
            tentacle_secret_env="another-very-secure-key-for-tentacle-middleware"
        )
