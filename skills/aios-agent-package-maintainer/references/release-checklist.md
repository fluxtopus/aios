...
2. Add/update `.changes/*.yaml` note for `pkg-aios-agent`.
3. Update `packages/aios-agent/CHANGELOG.md` when package-level release notes are required.
4. Ensure README examples and install instructions still match the API.

## Validation

1. Run unit tests: `pytest tests/unit -v` (from `packages/aios-agent`).
...
1. Confirm generated release PR includes `pkg-aios-agent` bump and `manifest.yaml` update.
2. Merge release PR.
3. Approve `Publish Release` environment gate.
4. Confirm publish target/index and credentials are configured.
5. Verify resulting tag `pkg-aios-agent@x.y.z` and archive manifest artifact.
...
2. Restore `pkg-aios-agent` version as needed.
